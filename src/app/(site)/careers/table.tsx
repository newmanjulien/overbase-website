      ...
      <div className="mt-2 space-y-1 md:space-y-2 text-sm">
        {OPENING.map((opening) => (
          <Link
            key={opening.id}
            href={opening.href}
            className="grid grid-cols-1 md:grid-cols-[minmax(0,1.35fr)_minmax(0,1.9fr)_minmax(0,0.6fr)_minmax(0,0.7fr)] gap-2 md:gap-6 px-3 md:px-6 py-3 md:py-6 -mx-1 md:-mx-3 items-center rounded-2xl transition-colors hover:bg-gray-50"
            target="_blank"
            rel="noreferrer"
          >
